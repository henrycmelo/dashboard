...
export default function MapPage() {
  ...
  const boroughs = ['All Boroughs', ...Array.from(new Set(clientData.map(item => item.borough)))]

  // Filter data
  const filteredData = clientData.filter(item => {
    const matchesBorough = boroughFilter === 'All Boroughs' || item.borough === boroughFilter
    const matchesSearch = item.zipCode.includes(searchTerm) || 
      ...
      <Box p={6}>
        ...
        >
          ...
          size="lg"
            ...
            <Table.Header>
              <Table.Row>
                <Table.ColumnHeader color="gray.600" fontSize="sm">Zip Code</Table.ColumnHeader>
                <Table.ColumnHeader color="gray.600" fontSize="sm">Borough</Table.ColumnHeader>
                <Table.ColumnHeader color="gray.600" fontSize="sm" textAlign="right">Clients</Table.ColumnHeader>
              </Table.Row>
            </Table.Header>
            <Table.Body>
              {paginatedData.map((item) => (
                <Table.Row key={`${item.zipCode}-${item.borough}`}>
                  ...
                  <Table.Cell fontSize="sm" textAlign="right">{item.clients.toLocaleString()}</Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table.Root>
          ...
          <Flex justify="space-between" align="center" mt={4}>
            <Text fontSize="sm" color="brand.secondary">
              Showing {startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredData.length)} of {filteredData.length} zip codes
            </Text>
